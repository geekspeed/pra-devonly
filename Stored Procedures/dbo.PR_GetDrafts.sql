...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[PR_GetDrafts]
@UserID varchar(30),
@UserRole varchar(30),
@PageSize int,
@GetPage int
...
IF UPPER(@UserRole)='USER' OR UPPER(@UserRole)='COO'
	BEGIN
		set @SearchCriteria = ' select  PRNum, RequestorId,Title, PRType,RequestorDept, SupplierName,  PRValue,convert(varchar(10), CreatedDate,101) CreatedDate, CreatedBy,isnull(RequestedDate,'''') RequestedDate
 from vw_intl_PurchaseReq where ( requestorId='''+@UserId+'''or createdby='''+@UserId+''' ) and StatusValue  = ''DRAFT''' 
	END
ELSE
	BEGIN
		set @SearchCriteria = 'select PRNum,RequestorId,Title,PRType,RequestorDept, SupplierName, PRValue,convert(varchar(10), CreatedDate,101) CreatedDate, CreatedBy,isnull(RequestedDate,'''') RequestedDate
 from vw_intl_PurchaseReq where  StatusValue =''DRAFT'' ' 
	END

--set @CountSql = 'select count(*) from vw_intl_PurchaseReq PR '
set @CountSql = 'select count(*) from ('+@SearchCriteria+') PR '

set @SelectSql = 'Select PRNum,isnull(AD.Name,RequestorId) RequestorId,Title,PRType,RequestorDept,SupplierName,PRValue,CreatedBy,CreatedDate ,Show,RequestedDate
from (select ROW_NUMBER() OVER(ORDER BY PR.RequestorId ASC) AS rownum
,PR.PRNum,PR.RequestorId,PR.Title,PR.PRType,PR.RequestorDept,PR.SupplierName,PR.PRValue,convert(varchar(10),PR.CreatedDate,101) CreatedDate,PR.CreatedBy,isnull(PRA.Show,''false'') Show , PR.RequestedDate 
 from ('+@SearchCriteria +') PR 
left outer join
