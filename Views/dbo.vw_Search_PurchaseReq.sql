...
SET ANSI_NULLS ON
GO
CREATE VIEW [dbo].[vw_Search_PurchaseReq]
as
SELECT 
PRN.PRNum, 
...
PR.Comments, PR.ShipName,
PR.Intl_PONumber as Intl_PONumber
FROM  
dbo.tbl_PurchaseReq AS PR INNER JOIN
...
dbo.tbl_Lookup_Values AS LV ON PR.GLCode = LV.LookupValue LEFT OUTER JOIN
 (SELECT PRID, CAST(ISNULL(SUM((100.00 - PartDiscount) * PartQty * PartUnitPrice * LocalToUsd / 100.00), 0) AS decimal(25, 2)) AS PRValue
