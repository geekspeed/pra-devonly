...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [dbo].[func_GetSupervisor] (@userid varchar(50),@level int)
returns varchar(50)
as
Begin
	...
	Begin
		...
		--print @SUPERVISOR
		if exists(select 1 from vw_Employee where EmployeeLoginId=@SUPERVISOR and EmployeeLevel=@level) AND (@SUPERVISOR <> @EMPGUY) 
		Begin
